...
scene_t scene; // variable for scene
float color_array[width][height][3]; // place array colors for printing

/* Actual ray tracing function */
/* Will be threaded */
void *
ray_trace( void* thread_id )
{
    ...
    for( int px=*((int*)thread_id); px<width; px+=nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
			...
			/* Add scaled colors to array for printing */
			color_array[px][py][0] = scaled_color[0];
			color_array[px][py][1] = scaled_color[1];
			color_array[px][py][2] = scaled_color[2];
...
main( int argc, char **argv )
{
    ...
    int thread_id_array[ nthreads ];

    // Call threaded ray_trace
    for(int i = 0; i < nthreads; i++) {
        thread_id_array[i] = i;
        int ret = pthread_create( &thread_array[i], NULL, ray_trace, &thread_id_array[i] );
        if(ret) {
	...
	for(int i = 0; i < height; i++) {
		for(int j = 0; j < width; j++) {
             ...
             */
            printf( "%.0f %.0f %.0f\n",
		    color_array[i][j][0], color_array[i][j][1], color_array[i][j][2] );
		}
		printf( "\n" );
	}
